...
def run_tests(matrix, N_atoms):
    ...
    # NANI tests
    t_start = time.perf_counter()
    mod = KmeansNANI(matrix, 10, 'MSD', N_atoms=N_atoms, init_type='comp_sim', percentage=10)
    initiators = mod.initiate_kmeans()
    kmeans = KMeans(n_clusters=len(initiators), init=initiators, n_init=1, random_state=None)
    kmeans.fit(matrix)
    t_end = time.perf_counter()
    t_dif = t_end-t_start
